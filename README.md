...
## Project Overview
Establish a robust **Continuous Integration and Continuous Deployment (CI/CD)** pipeline tailored for legacy applications. For detailed developer workflows, coding standards, and template usage, please refer to the [Application Code Flow Documentation](codeflow/codeflow.md).
...
- **Immutable Infrastructure (AMI Baking)**: Automated creation of Application Golden AMIs (Amazon Machine Images) with pre-configured dependencies and application code, derived from base Golden AMIs.
...
3.  **AMI Baking**: Packer spins up a temporary instance using the latest base **Golden AMI** from the **AWS Marketplace**, fetches the application artifact from **Artifactory**, installs dependencies, and **deploys the application**. This creates an **Application Golden AMI** for future deployments.
4.  **Infrastructure Provisioning**: Terraform updates the infrastructure configuration (Launch Templates) to use the new Application Golden AMI and triggers the creation of new instances.
...
graph TD
    ...
    F -->|Fetch Base Golden AMI| G[AWS Marketplace]
    F -->|Fetch Artifact| E
    F --> H[Install Dependencies & Deploy App]
    H --> I[Create Application Golden AMI]
